...
const loadDeps = (() => {
  ...
  return () => {
    if (!cache) {
      const d3Url = "https://cdn.jsdelivr.net/npm/d3@7.9.0/+esm";
      const pixiUrl = "https://cdn.jsdelivr.net/npm/pixi.js@7.4.2/+esm";
      const tweenUrl =
        "https://cdn.jsdelivr.net/npm/@tweenjs/tween.js@25.0.0/dist/tween.esm.js";
      cache = Promise.all([
        import(/* @vite-ignore */ d3Url),
        import(/* @vite-ignore */ pixiUrl),
        import(/* @vite-ignore */ tweenUrl),
      ]);
    }
    return cache;
  };
...
function fetchDataPromise(): Promise<Record<string, ContentDetails> | undefined> {
  const fetchData = (window as typeof window & {
    fetchData?: Promise<Record<string, ContentDetails> | undefined>;
  }).fetchData;

  if (fetchData) return fetchData;

  return fetch("/static/context-index.json")
    .then((response) => response.json())
    .catch(() => undefined);
}

function getFullSlug(): string {
  return simplifySlug(document.body.dataset.slug || window.location.pathname || "index");
}
...
function resolveRelative(_currentSlug: string, targetSlug: string): string {
  const slug = simplifySlug(targetSlug);
...
function registerEscapeHandler(container: HTMLElement, handler: () => void) {
...
function graphOptions(graph: HTMLElement): GraphOptions {
  const isGlobal = graph.classList.contains("global-graph-container");
  const parsed = JSON.parse(graph.dataset["cfg"] || "{}") as Partial<GraphOptions>;
...
async function renderGraph(graph: HTMLElement, fullSlug: string) {
  ...
  for (const [source, details] of data.entries()) {
    ...
    for (const dest of outgoing) {
      const target = simplifySlug(dest);
  ...
  if (!neighbourhood.has(slug) && validLinks.has("index")) {
  ...
  tweens.forEach((activeTween) => activeTween.stop());
  tweens.clear();
    ...
    select(app.view as HTMLCanvasElement).call(
      zoom()
        ...
        .on("zoom", ({ transform }: any) => {
          ...
          const scaleOpacity = Math.max((scaleAmount - 1) / 3.75, 0);
  ...
  function animate(time: number) {
    ...
    tweens.forEach((activeTween) => activeTween.update(time));
    try {
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => void bootGraph());
} else {
  void bootGraph();
}
